...
Water quality parameters ranges have been classified using a traffic light scheme of green, amber and red to indicate whether the variable is within acceptable limits. These limits are dependent upon the department. Comments will be made for each variable within each department highlighting if the variable is seen to be within the green,  amber or red classifications. Where occurunces are observed above optimal conditions, number of occurrences will be reported - each occurrence represents one tank day, although realistically each variable measured represents a snapshot of the conditions on any given day due to the potential for rapid change in water quality parameters/ bacterial counts. 
...
Clr range not defined by traffic light system. A quick literature search indicates safe levels of residual chlorine to be 0.24mg/l, 0.48mg/l and 0.57mg/l at salinities of 15ppt, 25ppt and 35ppt respectively. Levels typically appropriate, but check with neutralisation protocol to ensure this. (Cheng S-Y et al. 2013. Acute toxicity of residual chlorine on the white shrimp Litopeneaus vannamei juveniles at different salinity levels. J. Fish. Soc. Taiwan, 40(3): 183-191.)
...
Alkalinity; green 150-200, amber 110-149 or 201-240, red <110 or >240. Less than 50% of results within the green classification, alkalinity appears to be higher than desired for a large proportion of the time. Alkalinity stress may result in lower growth rates and down-regulation of immune related genes.
...
TAN; green <1, amber 1.1-3, red >3. TAN range of 1.0-<1.1 not defined in classification range; amber range modified to 1 - 3 for this analysis. Less than 20% of the results within the green classification and 50% of the results within the red classification. Cause for concern, investigate in greater detail.
...
NH~3~; green <0.16, amber 0.17-0.3, red >0.3. Classification not defined for values 0.16- <0.17, amber range modified to 0.16-0.3 for the purposes of this analysis. Less than 25% of results within green classification, greater than 50% of results within red classification. Salinity impacts toxicity of NH~3~, 24, 48, 72, 96h LC50 values considered at 25ppt salinity considered to be 2.93, 2.16, 1.91, 1.57 mg/l. Upper value of 6.2 may be made in error. Ammonia management is required, depression in immune response due to ammonia stress may increase susceptibility to vibrio.
...
wqp.maturation.summary <- water.quality23.join %>%
  filter(department.x == "Maturation") %>%
  skim(ph, salinity, alkalinity, tan, nh4, nh3, no2, do, temp) %>%
  select(-c(skim_type, n_missing, complete_rate))
```
...
Variation in counts among tank groups; tanks sandworms and nauplii tanks appear high.  Time series indicates a maturation tank (16) with a typical p0-p75 with a high maximum value of 30,000.
...
The understanding is that tank water exchanges are performed once per day when tank cleaning is also undertaken. Six feeds are given per day, at even intervals. There is the potential for TAN-N to accumulate during the day as feed is progressively metabolised. One can predict the maximum level of accumulated TAN-N from two perspectives (starting the tank at a theoretical starting point of zero, or estimating residual TAN-N from the previous day post water exchange). Maximum daily water exchanges appear to be 55% of tank volume.

``` {r nurseryTanksPredictedTan, fig.cap = "Sichon nursery tanks predicted TAN daily accumulation level (from daily starting point of zero mg/l TAN)", fig.align = "center", fig.dim = c(8,3), dpi = 200}
...
Estimated maximum daily accumulation of TAN-N (if starting from zero TAN):  
Tanks zone A/B `r nursery.tan.max[1,2] * 1e6 / (tanks_ab * 1000)` mg/l.  
Tanks zone C/D `r nursery.tan.max[2,2] * 1e6 / (tanks_cd * 1000)` mg/l.  

Estimated maximum daily accumulation of TAN-N (if not starting from zero TAN):  
Tanks zone A/B `r nursery.tan.max[1,2] * 1e6 / (tanks_ab * 1000) + 0.45 * (nursery.tan.max[1,2] * 1e6 / (tanks_ab * 1000))` mg/l.  
Tanks zone C/D `r nursery.tan.max[2,2] * 1e6 / (tanks_cd * 1000) + 0.4 * (nursery.tan.max[2,2] * 1e6 / (tanks_cd * 1000))` mg/l.
...
``` {r nurseryTanksModdeledWaterUse, fig.cap = "Sichon nursery tanks modelled water requirements based upon interpretation of production plan", fig.align = "center", fig.dim = c(8,3), dpi = 200}
...
```{r maturationLayout, out.width = "85%", fig.cap = "Maturation Layout", fig.align = "center", fig.dim = c(8,3), dpi = 200}
include_graphics("figs/maturation_layout.pdf") 
```
...
Summary data allows a quick visualisation for areas worth further investigation - pH looks OK, probably worth looking at values in maturation and leave it at that. Salinity variable - but not really a major issue for health. Alk - is that a typo, check the data type and see if it is an isolated figure. TAN - looks like a serious problem. This is where the focus is likely to be so far. Investigate tanks individually  - is it a common problem or isolated. Also plot detailed histogram for better visualisation of parameter distribution.

## Example plot

``` {r nurseryTank1tan, fig.cap = "TAN (mg/l) within Sichon nursery tank 1.", fig.align = "center", fig.dim = c(8,3), dpi = 200}
...
```{r tanDensityHistNursery, fig.cap = "Density histogram summarising frequency distribution of TAN (mg/l) as recorded in Sichon nursery.", fig.align = "center", fig.dim = c(8,3), dpi = 200}

freqDistNurTAN <- tan.plot.nursery.1 <- water.quality23.join %>%
  filter(department.x == "Nursery" & tank == "1") %>%
  ggplot(aes(x=tan, )) +
  geom_histogram(aes(y=..density..), binwidth = 0.5, alpha=0.4, position="identity") +
  geom_density(alpha=0.2, bw = 0.5) +
  geom_vline(xintercept = c(1, 3), linetype = c("dashed", "dotted"),
             color = c("orange", "red"), size = 1.2) +
  labs(x="TAN (mg/l)", y = "Frequency") +
  theme_minimal()

freqDistNurTAN
```

I wonder what time of day that the water quality readings are taken (just before the water exchange?)
